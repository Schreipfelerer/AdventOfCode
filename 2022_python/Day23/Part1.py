...
def solve(data):  # solves the question
        ...
        for x, row in enumerate(data):
            for y, c in enumerate(row):
                if c == "#":
                    ...
                    if next_n:
                        consider = "NSWENSWE"
                        to_consider = consider[i % 4: i % 4 + 4]
    ...
    while "#" not in data[-1]:
        data = data[:-1]
    while "#" not in [data[x][0] for x in range(len(data))]:
        data = [row[1:] for row in data]
    while "#" not in [data[x][-1] for x in range(len(data))]:
        data = [row[:1] for row in data]

    return sum([sum([1 if c == "." else 0 for c in row]) for row in data])
