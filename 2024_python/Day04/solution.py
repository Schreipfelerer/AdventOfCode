...
def solvePart1(data):  # solves the question
    ...
    joined = "\n".join(data)
    rotated = [
        "".join([data[y][x] for y in range(len(data))]) for x in range(len(data[0]))
    ]
    joined += "\n\n" + "\n".join(rotated)
    diagonal = [
        "".join(
            [
                data[
                    min(y, len(data) - 1)
                    - min(len(data[0]), len(data[0]) + len(data) - y - 1)
                    + x
                    + 1
                ][x]
                for x in range(
                    max(0, len(data[0]) - y - 1),
                    min(len(data[0]), len(data[0]) + len(data) - y - 1),
                )
            ],
        )
        for y in range(len(data) + len(data[0]) - 1)
    ]
    joined += "\n\n" + "\n".join(diagonal)
    data = data[::-1]
    diagonal2 = [
