...
def solvePart1(data):  # solves the question
    safe_num = 0
    for d in data:
        if all(map(lambda x: 0<x[0]-x[1]<4, zip(d[:-1], d[1:]))):
            safe_num += 1
        elif all(map(lambda x: 0<x[1]-x[0]<4, zip(d[:-1], d[1:]))):
            safe_num += 1
    return safe_num
...
def solvePart2(data):  # solves the question
    ...
    for d in data:
        if d[0] < d[-1]:
            d = d[::-1]
        bool_list = list(map(lambda x: 0 < x[0] - x[1] < 4, zip(d[:-1], d[1:])))
        ...
        if (all(map(lambda x: 0 < x[0] - x[1] < 4, zip(d_1[:-1], d_1[1:]))) or
                all(map(lambda x: 0 < x[0] - x[1] < 4, zip(d_2[:-1], d_2[1:])))):
            safe_num += 1
    return safe_num
